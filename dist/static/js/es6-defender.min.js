...
var easing = 0.05;
var playerAccelX = 0.2;
var playerDampingX = 0.1;
var playerMaxSpeedX = 1.9;
var playerMaxSpeedY = 0.7;
var debrisDamping = 0.01;
var modulusx = 512;
var halfmodulusx = modulusx / 2;
var starmodulusx = 256;
var halfstarmodulusx = starmodulusx / 2;
var projectileLifetime = 30;
...
var Star = function (_StateVector6) {
  _inherits(Star, _StateVector6);

  function Star(id, x, y, depth) {
    _classCallCheck(this, Star);

    var _this6 = _possibleConstructorReturn(this, (Star.__proto__ || Object.getPrototypeOf(Star)).call(this, id, x, y));
...
var updateDebrisPosition = function updateDebrisPosition(d) {
...
var updateDebris = function updateDebris(debris) {
  return debris.map(updateDebrisPosition);
};
...
var invaders = fillWith(10, function (_) {
  return new Invader(invaderId++, (Math.random() * 0.8 + 0.2) * halfmodulusx * [1, -1][Math.floor(Math.random() * 2)], 96 / 2, InvaderState.seeking, 0);
});
var humanId = 200;
var humans = fillWith(10, function (_) {
  return new Human(humanId++, (Math.random() - 0.5) * modulusx, 94, 0.2 * (Math.random() - 0.5));
});
var projectileId = 500;
var projectiles = [];
var invaderProjectileId = 1000;
var invaderProjectiles = [];
var starfield = fillWith(50, function (_) {
  return new Star(2000, (Math.random() - 0.5) * starmodulusx, Math.random() * 96, Math.random() * 0.5 + 0.5);
});
var debrisId = 3000;
var debris = [];
...
var score = 0;
var colours = new Map([[Player.graphic[0], Player.colour], [Player.graphic[1], Player.colour], [Invader.graphic, Invader.colour], [Invader.graphicAbducting, Invader.colourAbducting], [Human.graphic, Human.colour], [Projectile.graphic, Projectile.colour], [Projectile.graphic2, Projectile.colour2], [Debris.graphic, Debris.colour], [Star.graphic, Star.colour]]);

var doGame = function doGame(fastTextMode, viewWidth, viewHeight, input, t) {
  ...
  var allEvents = [].concat(projectileEvents, playerProjectileHitEvent, playerInvaderHitEvent, playerHumanHitEvent, invaderEvents, debrisEvents);
  ...
  starfield.map(function (s) {
    return graphics.set(s.id, Star.graphic);
  });
  debris.map(function (d) {
    return graphics.set(d.id, Debris.graphic);
  });
  ...
  var displacementList = [].concat(invaders, humans, projectiles, invaderProjectiles, debris);
  ...
  displayList.map(toLocal).filter(clip).map(function (i) {
    var g = graphics.has(i.id) ? graphics.get(i.id) : '!';
    var c = colours.has(g) ? colours.get(g) : 0xffffff;
