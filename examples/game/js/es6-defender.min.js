...
var easing = 0.05;
var playerAccel = 0.15;
var playerDamping = 0.1;
var halfmodulusx = 256;
var modulusx = 512;
...
var Human = function (_StateVector3) {
  ...
  function Human(id, x, y, xdot) {
    _classCallCheck(this, Human);

    return _possibleConstructorReturn(this, (Human.__proto__ || Object.getPrototypeOf(Human)).call(this, id, x, y, xdot));
  }
...
var updateInvaderPosition = function updateInvaderPosition(sv, state, targetx, targety) {
  ...
  var a = (_InvaderState$seeking = {}, _defineProperty(_InvaderState$seeking, InvaderState.seeking, function () {
    ...
    sv.x += sv.xdot;
    sv.y += sv.ydot;
  }), _defineProperty(_InvaderState$seeking, InvaderState.abducted, function () {
    sv.ydot = -0.1;

    sv.y += sv.ydot;
  }), _InvaderState$seeking)[state]();

  return sv;
};

var updateInvaders = function updateInvaders(invaders, invaderTargets) {
...
var updateInvaderState = function updateInvaderState(invaders, id, state) {
  var i = invaders.findIndex(function (i) {
    return i.id == id;
  });
...
var checkLockedInvader = function checkLockedInvader(invader, e) {
  return invader.y + Invader.sideLen >= Global.viewHeight ? [{ event: Event.abducted, invaderId: invader.id, humanId: e.humanId }] : [];
};
...
var checkHitInvaders = function checkHitInvaders(invaders, projectiles) {
  return detectCollisions(invaders, Invader.sideLen, projectiles, Projectile.sideLen).map(function (collidedPair) {
    collidedPair.event = Event.dead;return collidedPair;
  });
};
...
var checkProjectiles = function checkProjectiles(projectiles, t) {
  return projectiles.filter(function (p) {
    ...
    return { event: Event.remove, id: p.id };
  });
};
...
var playerId = 1;
var invaderId = 100;
var player = new Player(playerId, 0, 48 / 2, PlayerState.faceRight, 0);
var invaders = initArray(3, function (_) {
  return new Invader(invaderId++, Math.floor(Math.random() * modulusx), 48 / 2, InvaderState.seeking, 0);
});
var humanId = 200;
var humans = initArray(10, function (_) {
  return new Human(humanId++, Math.floor(Math.random() * modulusx), 47, 0.2 * (Math.random() - 0.5));
});
...
var doGame = function doGame(fastTextMode, viewWidth, viewHeight, input) {
  ...
  var seekingInvaderEvents = invaders.filter(function (i) {
    return i.state == InvaderState.seeking;
  }).reduce(function (arr, i) {
    return arr.concat(checkSeekingInvader(i, humans));
  }, []);
  var lockedInvaderEvents = invaders.filter(function (i) {
    return i.state == InvaderState.locked;
  }).reduce(function (arr, i) {
    return arr.concat(checkLockedInvader(i, invaderTargets.get(i.id)));
  }, []);

  var allEvents = [].concat(projectileEvents, hitEvents, seekingInvaderEvents, lockedInvaderEvents);
  allEvents.filter(function (e) {
    return e.event == Event.remove;
  }).map(function (e) {
    return remove(projectiles, e.id, graphics);
  });
  allEvents.filter(function (e) {
    return e.event == Event.dead;
  }).map(function (e) {
    return remove(invaders, e.id1, graphics);
  });
  allEvents.filter(function (e) {
    return e.event == Event.locked;
  }).map(function (e) {
    updateInvaderState(invaders, e.invaderId, InvaderState.locked);invaderTargets.set(e.invaderId, e);
  });
  allEvents.filter(function (e) {
    return e.event == Event.abducted;
  }).map(function (e) {
    console.log('hello');updateInvaderState(invaders, e.invaderId, InvaderState.abducted);
  });

  graphics.set(player.id, player.state == PlayerState.faceLeft ? Player.graphic[0] : Player.graphic[1]);

  invaders.map(function (i) {
    return graphics.set(i.id, Invader.graphic);
  });
  ...
  var displacementList = [].concat(invaders, humans, projectiles);
