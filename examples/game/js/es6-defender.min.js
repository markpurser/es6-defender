...
var doGame = function doGame(fastTextMode, viewWidth, viewHeight, input) {
  ...
  projectileEvents.map(function (pe) {
    var i = projectiles.find(function (n) {
      return n.id == pe.id;
    });
  ...
  displayArray.map(toLocal).filter(clip).map(function (i) {
    return fastTextMode.setString(Math.floor(i.lx), Math.floor(i.ly), graphics.has(i.id) ? graphics.get(i.id) : '!');
  });
