...
var doGame = function doGame(fastTextMode, viewWidth, viewHeight, input) {
  ...
  projectileEvents.map(function (pe) {
    var i = projectiles.findIndex(function (n) {
      return n.id == pe.id;
    });
  ...
  var hits = checkHitInvaders(invaders, projectiles);
  if (hits.length > 0) {
    var id1 = hits[0].id1;
    if (id1 >= 100 && id1 < 200) {
      var i = invaders.findIndex(function (n) {
        return n.id == id1;
      });
      invaders.splice(i, 1);
      graphics.delete(id1);
  ...
  }), humans.map(updateHumanPosition), projectiles.map(updateProjectilePosition));
